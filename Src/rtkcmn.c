...
const double leaps[][8]={ /* leap seconds {y,m,d,h,m,s,utc-gpst,...} */
    {2015,7,1,0,0,0,-17},
		{2012,7,1,0,0,0,-16},
    {2009,1,1,0,0,0,-15},
    {2006,1,1,0,0,0,-14},
...
const double lam[]={            /* carrier wave length (m) */
    CLIGHT/FREQ1,CLIGHT/FREQ2,CLIGHT/FREQ5,CLIGHT/FREQ6,CLIGHT/FREQ7,CLIGHT/FREQ8,CLIGHT/FREQ2_CMP,CLIGHT/FREQ7_CMP,CLIGHT/FREQ6_CMP
};
...
// 修改要慎重
const prcopt_t prcopt_gps_doul={ /* defaults processing options */
	PMODE_KINEMA,0,1,SYS_GPS|SYS_CMP|0,		/* mode,soltype,nf,navsys */
	15.0*D2R,{{0,0}},						/* elmin,snrmask *///程序中会有相关判断,会限制到15以下
	0,ARMODE_FIXHOLD,1,5,0,10,				/* sateph,modear,glomodear,maxout,minlock,minfix *///ARMODE_FIXHOLD
    ...
    {100.0,0.003,0.003,0.0,1.0},			/* err[] */
    {30.0,0.03,0.3},						/* std[] */
    {1E-4,1E-3,1E-4,1E1,1E1},				/* prn[] */
    5E-12,									/* sclkstab */
    {3.0,0.9999,0.2},							/* thresar */
    10.0*D2R,10.0*D2R,0.0,					/* elmaskar,almaskhold,thresslip */
    10.0,0.0,30.0,							/* maxtdif,maxinno,maxgdop */
    {0},{0},{0},							/* baseline,ru,rb */
    {"",""},								/* anttype */
    {{0}},{{0}},{0}							/* antdel,pcv,exsats */
};
...
/* crc 24 table generated by util/gencrc -------------------------------------*/
...
*-----------------------------------------------------------------------------*/
//#pragma CODE_SECTION(satid2no,"sect_ECODE_I");
...
* return : none
*-----------------------------------------------------------------------------*/
//#pragma CODE_SECTION(satno2id,"sect_ECODE_I");
...
//        LOG_printf(&trace,"unhealth sat=%3d svh=%02X",sat);
        return 1;
    }
    return 0;
}
...
*-----------------------------------------------------------------------------*/
//#pragma CODE_SECTION(obs2code,"sect_ECODE_I");
...
* notes  : obs codes are based on reference [6] and qzss extension
*-----------------------------------------------------------------------------*/
//#pragma CODE_SECTION(code2obs,"sect_ECODE_I");
...
* return : none
*-----------------------------------------------------------------------------*/
//#pragma CODE_SECTION(setcodepri,"sect_ECODE_I");
...
*-----------------------------------------------------------------------------*/
//#pragma CODE_SECTION(getcodepri,"sect_ECODE_I");
...
*-----------------------------------------------------------------------------*/
//#pragma CODE_SECTION(getbitu,"sect_ECODE_I");
...
//#pragma CODE_SECTION(getbits,"sect_ECODE_I");
...
* return : none
*-----------------------------------------------------------------------------*/
//#pragma CODE_SECTION(setbitu,"sect_ECODE_I");
...
//#pragma CODE_SECTION(setbits,"sect_ECODE_I");
...
*-----------------------------------------------------------------------------*/
//#pragma CODE_SECTION(crc32,"sect_ECODE_I");
...
*-----------------------------------------------------------------------------*/
//#pragma CODE_SECTION(crc24q,"sect_ECODE_I");
...
*-----------------------------------------------------------------------------*/
//#pragma CODE_SECTION(decode_word,"sect_ECODE_I");
...
*-----------------------------------------------------------------------------*/
//#pragma CODE_SECTION(mat,"sect_ECODE_I");
...
* return : matrix pointer (if n<=0 or m<=0, return NULL)
*-----------------------------------------------------------------------------*/
//#pragma CODE_SECTION(imat,"sect_ECODE_I");
...
* return : matrix pointer (if n<=0 or m<=0, return NULL)
*-----------------------------------------------------------------------------*/
//#pragma CODE_SECTION(zeros,"sect_ECODE_II");
...
*-----------------------------------------------------------------------------*/
//#pragma CODE_SECTION(eye,"sect_ECODE_I");
...
//#pragma CODE_SECTION(eye_wy,"sect_ECODE_I");
extern void eye_wy(int n,double *p)
{
    //double *p;
    int i;
...
* return : none
*-----------------------------------------------------------------------------*/
extern void matmul_wy(const char *tr, int n, int k, int m, double alpha,
                   const double *A, const double *B, double beta, double *C)
{
    int lda=tr[0]=='T'?m:n,ldb=tr[1]=='T'?k:m;
                   ...
                   const double *A, const double *B, double beta, double *C)
{
	...
	/*static int N=0,K=0,M=0;
	if(n>N) N=n;
	if(k>K) K=k;
	if(m>M) M=m;
    
    #pragma MUST_ITERATE(0,50);
	for (i=0;i<n;i++) 
	{
		#pragma MUST_ITERATE(0,50);
		for (j=0;j<k;j++) 
...
//#pragma DATA_SECTION(dMat_vv,"sect_EDATA_I");
//double dMat_vv[1*147]={0.0};
static int ludcmp(double *A, int n, int *indx, double *d)
{
    double big,s,tmp,*vv=mat(n,1);
	//double big,s,tmp,*vv=dMat_vv;
    int i,imax=0,j,k;
    //LOG_printf(&trace,"ludcmp start");
    *d=1.0;
    for (i=0;i<n;i++) {
...
//#pragma DATA_SECTION(iMat_indx_matinv,"sect_EDATA_I");
//#pragma DATA_SECTION(dMat_B_matinv,"sect_EDATA_I");
//double dMat_B_matinv[130*147]={0.0};
//int iMat_indx_matinv[1*147]={0};

extern int matinv(double *A, int n)
{
    double d,*B;
    int i,j,*indx;
    	//LOG_printf(&trace,"matinv start");
    	
    indx=imat(n,1); B=mat(n,n); matcpy(B,A,n,n);

	//indx=iMat_indx_matinv; B=dMat_B_matinv; matcpy(B,A,n,n);
...
/* solve linear equation -----------------------------------------------------*/
//#pragma DATA_SECTION(dMat_B_solve,"sect_EDATA_I");
//double dMat_B_solve[130*147]={0.0};
extern int solve(const char *tr, const double *A, const double *Y, int n,
                 int m, double *X)
{
    double *B=mat(n,n);
	//double *B=dMat_B_solve;
    int info;
//    LOG_printf(&trace,"slove start");
    matcpy(B,A,n,n);
...
//#pragma CODE_SECTION(matinv_pd,"sect_ECODE_I");
//#pragma DATA_SECTION(dMat_b,"sect_EDATA_I");
//double dMat_b[1*147]={0.0};
//double dTempTic[30];
extern int matinv_pd(double *a, int n)
{
	int i,j,k,m;
	double w, g, *b;
	//LOG_printf(&trace,"matinv pd start");
	//b = (double*)malloc(n*sizeof(double));
//	dTempTic[3]			= PRD_getticks();
	b=mat(n,1);
	//b=dMat_b;
	if (n<=0||n>2000)
	{free(b);return 0;}
//	dTempTic[4]			= PRD_getticks();
	for (k=0; k<n; k++)
	{
		w=a[0];
		if (fabs(w) + 1.0 == 1.0)
...
//	dTempTic[5]			= PRD_getticks();
	for (i=0; i<n-1; i++)
		for (j=i+1; j<n; j++)
			a[i*n+j]=a[j*n+i];
	free(b);
//	dTempTic[6]			= PRD_getticks();
	return 0;
}
...
*-----------------------------------------------------------------------------*/
//#pragma CODE_SECTION(lsq,"sect_ECODE_I");
//#pragma DATA_SECTION(dMat_Ay,"sect_EDATA_I");
//double dMat_Ay[1*147]={0.0};
               ...
               double *Q)
{
	...
	if (!(info=matinv_pd(Q,n))) matmul("NN",n,1,n,1.0,Q,Ay,0.0,x); /* x=Q^-1*Ay */
    free(Ay);
    return info;
}
/* kalman filter ---------------------------------------------------------------
...
*-----------------------------------------------------------------------------*/
//#pragma CODE_SECTION(filter_,"sect_ECODE_I");
//#pragma DATA_SECTION(dMat_F,"sect_EDATA_I");
//#pragma DATA_SECTION(dMat_Q,"sect_EDATA_I");
//#pragma DATA_SECTION(dMat_K,"sect_EDATA_I");
//#pragma DATA_SECTION(dMat_I,"sect_EDATA_I");
//double dMat_F[130*147]={0.0},dMat_Q[130*147]={0.0},dMat_K[130*147]={0.0},dMat_I[130*147]={0.0};
                   ...
                   double *xp, double *Pp)
{
	...
	/*memset(dMat_F,0,sizeof(dMat_F));
	memset(dMat_Q,0,sizeof(dMat_Q));
	memset(dMat_K,0,sizeof(dMat_K));
	F=dMat_F;Q=dMat_Q;K=dMat_K;

	memset(dMat_I,0,sizeof(dMat_I));
	eye_wy(n,dMat_I);
	I = dMat_I;*/
    
    matcpy(Q,R,m,m);
    matcpy(xp,x,n,1);
//	dTempTic[0]			= PRD_getticks();
	
    matmul("NN",n,m,n,1.0,P,H,0.0,F);       /* Q=H'*P*H+R */
//	dTempTic[1]			= PRD_getticks();
    matmul("TN",m,m,n,1.0,H,F,1.0,Q);
//	dTempTic[2]			= PRD_getticks();
    if (!(info=matinv_pd(Q,m))) {
//if (!(info=matinv(Q,m))) {
//		dTempTic[7]			= PRD_getticks();
        matmul("NN",n,m,m,1.0,F,Q,0.0,K);   /* K=P*H*Q^-1 */
//		dTempTic[8]			= PRD_getticks();
        matmul("NN",n,1,m,1.0,K,v,1.0,xp);  /* xp=x+K*v */
//		dTempTic[9]			= PRD_getticks();
        matmul("NT",n,n,m,-1.0,K,H,1.0,I);  /* Pp=(I-K*H')*P */
//		dTempTic[10]			= PRD_getticks();
        matmul("NN",n,n,n,1.0,I,P,0.0,Pp);
//		dTempTic[11]			= PRD_getticks();
    }
...
//////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////
//#pragma CODE_SECTION(filter,"sect_ECODE_I");
//#pragma DATA_SECTION(dMat_x_,"sect_EDATA_I");
//#pragma DATA_SECTION(dMat_xp_,"sect_EDATA_I");
//#pragma DATA_SECTION(dMat_P_,"sect_EDATA_I");
//#pragma DATA_SECTION(dMat_Pp_,"sect_EDATA_I");
//#pragma DATA_SECTION(dMat_H_,"sect_EDATA_I");
//#pragma DATA_SECTION(iMat_xi,"sect_EDATA_I");
//double dMat_x_[1*147]={0.0},dMat_xp_[1*147]={0.0},dMat_P_[130*147]={0.0},dMat_Pp_[130*147]={0.0},dMat_H_[130*147]={0.0};
//int iMat_xi[1*147]={0};
                  ...
                  const double *R, int n, int m)
{
	...
	//memset(dMat_x_,0,sizeof(dMat_x_));
	//memset(dMat_xp_,0,sizeof(dMat_xp_));
	//memset(dMat_P_,0,sizeof(dMat_P_));
	//memset(dMat_Pp_,0,sizeof(dMat_Pp_));
	//memset(dMat_H_,0,sizeof(dMat_H_));
	//memset(iMat_xi,0,sizeof(iMat_xi));
	
	//ix=iMat_xi; for (i=k=0;i<n;i++) if (x[i]!=0.0&&P[i+i*n]>0.0) ix[k++]=i;
    //x_=dMat_x_; xp_=dMat_xp_; P_=dMat_P_; Pp_=dMat_Pp_; H_=dMat_H_;
...
//#pragma CODE_SECTION(filter_wei_,"sect_ECODE_I");
static int filter_wei_(const double *x, const double *P, const double* H1, const H_cell* H2,	const double *v, const double *R, int n, int m, int na,
	double *xp, double *Pp)
{
	double *Q=mat(m,m),*K=mat(n,m);
	int info, i, j, k;
	double d;
	double *PH=mat(n,m), *H_P_=mat(m,n), *H_PH=mat(m,m);
	...
	for (i=0; i<n; i++)
	{
		for (j=0; j<m; j++)
		{
			...
			for (k=0; k<na; k++)
				d += P[i + k*n] * H1[k+j*na];
			if (H2[j].hasValue)
				PH[i + j*n] = d + H2[j].val1 * P[i + n*(H2[j].pos1)] + H2[j].val2 * P[i + n*(H2[j].pos2)];
			else
				PH[i + j*n] = d;
			//H_P_[j + i*n] = PH[i + j*n];
		}
	...
	for (i=0; i<m; i++)
	{
		for (j=0; j<n; j++)
		{
			...
			for (k=0; k<na; k++)
				d += H1[k+i*na] * P[j + k*n];
			if (H2[i].hasValue)
				H_P_[i + j*m] = d + H2[i].val1 * P[j + n*(H2[i].pos1)] + H2[i].val2 * P[j + n*(H2[i].pos2)];
			else
				H_P_[i + j*m] = d;
		}
	}
	...
	for (i=0; i<m; i++)
	{
		for (j=0; j<m; j++)
		{
			d = .0;
			for (k=0; k<na; k++)
				d += H1[k+i*na] * PH[k + j*n];
			if (H2[i].hasValue)
				H_PH[i + j*m] = d + H2[i].val1 * PH[H2[i].pos1 + n*(j)] + H2[i].val2 * PH[H2[i].pos2 + n*(j)];
			else
				H_PH[i + j*m] = d;
		}
	}
	...
	for (i=0; i<m; i++)
		for (j=0; j<m; j++)
			Q[i + j*m] = R[i + j*m] + H_PH[i + j*m];

	matcpy(xp,x,n,1);
	matcpy(Pp, P, n, n);

	if (!(info=matinv(Q,m))) {
		matmul("NN",n,m,m,1.0,PH,Q,0.0,K);    /* K=P*H*Q^-1 */
		matmul("NN",n,1,m,1.0,K,v,1.0,xp);    /* xp=x+K*v */
		matmul("NN",n,n,m,-1.0,K,H_P_,1.0,Pp);/* Pp=(I-K*H')*P */
	}


	free(Q); free(K); free(PH); free(H_PH); free(H_P_);
	return info;
}

//#pragma CODE_SECTION(filter_wei,"sect_ECODE_I");
extern int filter_wei(double *x, double *P, const double *H, const double *v,
                  const double *R, int n, int m, int na)
{
	double *x_,*xp_,*P_,*Pp_; //,*H_;
	int i,j,k,info,*ix,w;
	//char* debugBuffer;
	...
	//debugBuffer = malloc(sizeof(char) * 5120);
	...
	//debug print
	/*	debugLen = 0;
	memset(debugBuffer, 0, 5120);
	for (w=0; w<k; w++)	{
		sprintf(debugBuffer + debugLen, "%d\n", ix[w]);
		debugLen = strlen(debugBuffer);
		}
	trace(1, "ix_2(i=%d)Befor=\n%s\n", i, debugBuffer); */



		for (j=0; j<m; j++)
			H1[i+j*na]=H[ix[i]+j*n];
	...
	//H2
	H2 = (H_cell*)malloc(sizeof(H_cell) * m);
	for (j=0; j<m; j++)
	{
		H2[j].hasValue = 0;
		for (i=na; i<k; i++)
	...
	debugLen = 0;
	for (i=0; i<k-na; i++)	{
		sprintf(debugBuffer + debugLen, "%d  [%d](%7.3lf) - [%d](%7.3lf)\n", H2[i].hasValue, H2[i].pos1, H2[i].val1, H2[i].pos2, H2[i].val2);
		debugLen = strlen(debugBuffer);
		}*/
	
    info=filter_wei_(x_,P_,H1,H2,v,R,k,m,na,xp_,Pp_);
    for (i=0;i<k;i++) {
        x[ix[i]]=xp_[i];
        for (j=0;j<k;j++) P[ix[i]+ix[j]*n]=Pp_[i+j*k];
	}
	...
	/*trace(1, "H1=\n");
	tracemat(1,H1,na,m,6,3);
	trace(1, "H2=\n%s\n", debugBuffer);

	trace(1, "Xp=\n");
	tracemat(1, xp_, k, 1, 6, 3);
	trace(1, "Pp=\n");
	tracemat(1, Pp_, k, k, 6, 3);*/

    free(ix); free(x_); free(xp_); free(P_); free(Pp_); free(H1); free(H2); //free(debugBuffer);
    return info;
}
/* smoother --------------------------------------------------------------------
...
*          matirix stored by column-major order (fortran convention)
*-----------------------------------------------------------------------------*/
//#pragma CODE_SECTION(smoother,"sect_ECODE_I");
//#pragma DATA_SECTION(dMat_invQf,"sect_EDATA_I");
//#pragma DATA_SECTION(dMat_invQb,"sect_EDATA_I");
//#pragma DATA_SECTION(dMat_xx,"sect_EDATA_I");
//double dMat_invQf[130*147]={0.0},dMat_invQb[130*147]={0.0},dMat_xx[1*147]={0.0};
extern int smoother(const double *xf, const double *Qf, const double *xb,
                    const double *Qb, int n, double *xs, double *Qs)
{
    double *invQf=mat(n,n),*invQb=mat(n,n),*xx=mat(n,1);
    //int i,info=-1;
	//double *invQf=dMat_invQf,*invQb=dMat_invQb,*xx=dMat_xx;
    int i,info=-1;
...
extern gtime_t epoch2time(const double *ep)
{
    ...
    sec=(int)floor(ep[5]);
    time.time=(long)days*86400+(int)ep[3]*3600+(int)ep[4]*60+sec;
    time.sec=ep[5]-sec;
    return time;
}
...
extern void time2epoch(gtime_t t, double *ep)
{
    ...
    sec=(int)(t.time-(long)days*86400);
...
//#pragma CODE_SECTION(addpcv,"sect_ECODE_I");
...
*-----------------------------------------------------------------------------*/
//#pragma CODE_SECTION(searchpcv,"sect_ECODE_I");
...
/* compare ephemeris ---------------------------------------------------------*/
//#pragma CODE_SECTION(cmpeph,"sect_ECODE_I");
...
//#pragma CODE_SECTION(uniqeph,"sect_ECODE_I");
...
//#pragma CODE_SECTION(cmpgeph,"sect_ECODE_I");
...
//#pragma CODE_SECTION(uniqgeph,"sect_ECODE_I");
...
//#pragma CODE_SECTION(cmpseph,"sect_ECODE_I");
...
//#pragma CODE_SECTION(uniqseph,"sect_ECODE_I");
...
*-----------------------------------------------------------------------------*/
//#pragma CODE_SECTION(uniqnav,"sect_ECODE_I");
...
//#pragma CODE_SECTION(cmpobs,"sect_ECODE_I");
...
//LOG_printf(&trace,"q1 %d %d",q1->rcv,q1->sat);
//LOG_printf(&trace,"q2 %d %d",q2->rcv,q2->sat);
...
* return : number of epochs
*-----------------------------------------------------------------------------*/
//#pragma CODE_SECTION(sortobs,"sect_ECODE_I");
...
//#pragma CODE_SECTION(sortobs_my,"sect_ECODE_I");
extern int sortobs_my(obs_t *obs,int start_n,int length)
{
    ...
    qsort((obs->data+start_n),length,sizeof(obsd_t),cmpobs);
...
*-----------------------------------------------------------------------------*/
//#pragma CODE_SECTION(screent,"sect_ECODE_I");
...
extern double satwavelen(int sat, int frq, const nav_t *nav)
{
    ...
    if (sys==SYS_GLO) {
		...
		if      (frq==0) return CLIGHT/FREQ2_CMP; /* B1 */
		else if (frq==1) return CLIGHT/FREQ7_CMP; /* B2 */
		else if (frq==2) return CLIGHT/FREQ6_CMP; /* B3 */
    }
    else {
...
*-----------------------------------------------------------------------------*/
//#pragma CODE_SECTION(ionppp,"sect_ECODE_I");
...
* return : none
*-----------------------------------------------------------------------------*/
//#pragma CODE_SECTION(antmodel_s,"sect_ECODE_I");
...
* return : none
*-----------------------------------------------------------------------------*/
//#pragma CODE_SECTION(csmooth,"sect_ECODE_I");
