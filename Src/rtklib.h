...
#define MAXSAT      46//(NSATGPS+NSATGLO+NSATGAL+NSATQZS+NSATCMP+NSATSBS)
                                        /* max satellite number (1 to MAXSAT) */
#ifndef MAXOBS
#define MAXOBS      46                  /* max number of obs in an epoch */
#endif
...
#define MAXERRMSG   1                /* max length of error/warning message */
...
#define ARMODE_CONT 1                   /* AR mode: continuous */ // 这三个是rtk
...
typedef struct {        /* receiver raw data control type */
    ...
    char msgtype[256];  /* last message type */
    unsigned char subfrm[MAXSAT][380];  /* subframe buffer (1-5) */
    ...
    int nbyte;          /* number of bytes in message buffer */ 
    int len;            /* message length (bytes) */
		int lenRev;            /* message length (bytes) */
    ...
    unsigned char buff[MAXRAWLEN]; /* message buffer */
		unsigned char buffRev[MAXRAWLEN]; /* message buffer */
...
extern const prcopt_t prcopt_gps_doul;
...
extern void setI4(unsigned char *p, int            i);
extern void setU2(unsigned char *p, unsigned short u);
