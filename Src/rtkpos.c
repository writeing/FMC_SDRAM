...
#define VAR_HOLDAMB 0.00001    /* constraint to hold ambiguity (cycle^2) */
...
//#pragma CODE_SECTION(sdobs,"sect_ECODE_II");
...
//#pragma CODE_SECTION(gfobs_L1L2,"sect_ECODE_II");
...
//#pragma CODE_SECTION(gfobs_L1L5,"sect_ECODE_II");
...
/* baseline length -----------------------------------------------------------*/
//#pragma CODE_SECTION(baseline,"sect_ECODE_II");
...
//#pragma CODE_SECTION(initx,"sect_ECODE_II");
...
//#pragma CODE_SECTION(selsat,"sect_ECODE_II");
	...
	for(i=0;i<nu;i++)
   	{
		for(j=nu;j<nu+nr;j++)
		{
			if( (obs[i].sat==obs[j].sat) && (azel[1+j*2]>=opt->elmin) )
			{
...
//#pragma CODE_SECTION(udpos,"sect_ECODE_II");
//#pragma DATA_SECTION(dMat_F_udpos,"sect_EDATA_I");
//#pragma DATA_SECTION(dMat_FP_udpos,"sect_EDATA_I");
//#pragma DATA_SECTION(dMat_xp__udpos,"sect_EDATA_I");
//double dMat_F_udpos[130*147]={0.0},dMat_FP_udpos[130*147]={0.0},dMat_xp__udpos[147]={0.0};
...
static void udpos(rtk_t *rtk, double tt)
{
	...
	//FP=dMat_FP_udpos; xp=dMat_xp__udpos;
	//memset(dMat_F_udpos,0,sizeof(dMat_F_udpos));
	//eye_wy(rtk->nx,dMat_F_udpos);
	//F=dMat_F_udpos; 
...
//#pragma CODE_SECTION(udion,"sect_ECODE_II");
...
//#pragma CODE_SECTION(udtrop,"sect_ECODE_II");
...
//#pragma CODE_SECTION(udrcvbias,"sect_ECODE_II");
...
//#pragma CODE_SECTION(detslp_ll,"sect_ECODE_II");
...
//            LOG_printf(&trace,"slip detected sat=%d rcv=%d LLI%d",sat,rcv,f+1);
        }
...
//#pragma CODE_SECTION(detslp_gf_L1L2,"sect_ECODE_II");
...
//        LOG_printf(&trace,"slip detected sat=%d GF_L1_L2)",sat);
    }
}
/* detect cycle slip by L1-L5 geometry free phase jump -----------------------*/
//#pragma CODE_SECTION(detslp_gf_L1L5,"sect_ECODE_II");
...
//        LOG_printf(&trace,"slip detected sat=%d GF_L1_L5)",sat);
    }
}
/* detect cycle slip by doppler and phase difference -------------------------*/
////#pragma CODE_SECTION(detslp_dop,"sect_ECODE_II");
...
////#pragma CODE_SECTION(udbias,"sect_ECODE_II");
double dMat_Zeros[147];
                   ...
                   const int *iu, const int *ir, int ns, const nav_t *nav)
{
	...
	static unsigned short usSvNums = 0,usSvNums2=0,usSvNums3=0;
    ...
    for (f=0;f<nf;f++) {
        ...
        for (i=1;i<=MAXSAT;i++) {
			...
			// 三个历元中，有任何一次的卫星数多余上一次的卫星数，并且上次的ratio<3.0，则清协方差阵
			// if( (rtk->opt.modear==ARMODE_CONT)&&( usSvNums<ns||usSvNums2<usSvNums || usSvNums3<usSvNums2 ) ||(g_usRtkRatio<2.0) )  // CHANGES
			// {
			// 	reset = 1;
			// }
            
			if(g_usRtkRatio<2.0)//为了避免有时候ulbox数据还没有ok,无效或者信号不太好,会导致固定不上,这样成功率会高些
			{
				reset = 1;
			}
        ...
        //bias=zeros(ns,1);
		memset(dMat_Zeros,0,sizeof(dMat_Zeros));
        bias=dMat_Zeros;
	...
	usSvNums3 	= usSvNums2;				// CHANGES
	usSvNums2 	= usSvNums;
	usSvNums 	= ns;
}
/* temporal update of states --------------------------------------------------*/
//#pragma CODE_SECTION(udstate,"sect_ECODE_II");
...
//#pragma CODE_SECTION(zdres_sat,"sect_ECODE_II");
...
//#pragma CODE_SECTION(zdres,"sect_ECODE_II");
                 ...
                 double *e, double *azel)
{
    ...
    for (i=0;i<n*nf*2;i++) y[i]=0.0;
    	//LOG_printf(&trace,"rr0 %d",(int)*(rr));
	//LOG_printf(&trace,"rr1 %d",(int)*(rr+1));
	//LOG_printf(&trace,"rr2 %d",(int)*(rr+2));
	//LOG_printf(&trace,"rr3 %d",(int)*(rr+3));
    if (norm(rr,3)<=0.0) 
    {
//		LOG_printf(&trace,"zdres no rec pos");
...
//#pragma CODE_SECTION(validobs,"sect_ECODE_II");
...
//#pragma CODE_SECTION(ddcov,"sect_ECODE_II");
...
//#pragma CODE_SECTION(constbl,"sect_ECODE_II");
...
//#pragma CODE_SECTION(prectrop,"sect_ECODE_II");
...
//#pragma CODE_SECTION(gloicbcorr,"sect_ECODE_II");
...
//#pragma CODE_SECTION(ddres,"sect_ECODE_II");
//#pragma DATA_SECTION(dMat_Ri,"sect_EDATA_I");
//#pragma DATA_SECTION(dMat_Rj,"sect_EDATA_I");
//#pragma DATA_SECTION(dMat_im,"sect_EDATA_I");
//#pragma DATA_SECTION(dMat_tropu,"sect_EDATA_I");
//#pragma DATA_SECTION(dMat_tropr,"sect_EDATA_I");
///#pragma DATA_SECTION(dMat_dtdxu,"sect_EDATA_I");
//#pragma DATA_SECTION(dMat_dtdxr,"sect_EDATA_I");
//double dMat_Ri[2500]={0.0},dMat_Rj[2500]={0.0},dMat_im[50]={0.0};
//double dMat_tropu[50]={0.0},dMat_tropr[50]={0.0},dMat_dtdxu[150]={0.0},dMat_dtdxr[150]={0.0};
int dIU[100],dIR[100];
...
//    LOG_printf(&trace,"ddres nx=%d ns=%d",rtk->nx,ns);
	...
	//Ri=dMat_Ri; Rj=dMat_Rj; im=dMat_im;
    //tropu=dMat_tropu; tropr=dMat_tropr; dtdxu=dMat_dtdxu; dtdxr=dMat_dtdxr;
    ...
    for (m=0;m<3;m++) /* for each system (0:gps/qzss/sbas,1:glonass,2:cmp) */
    {	
    for (f=opt->mode>PMODE_DGPS?0:nf;f<nf*2;f++) {
        ...
        }//end for (i=-1,j=0;j<ns;j++)
        if (i<0) continue;
		
        //LOG_printf(&trace,"reference sat %d",sat[i]);
		...
		//-------------------
			...
			//LOG_printf(&trace,"ion sati=%d satj=%d",sat[i],sat[j]);
			//LOG_printf(&trace,"ion HIi=%d Hij=%d",II(sat[i],opt),II(sat[j],opt));
                }
            }
			...
			//LOG_printf(&trace,"dd sati=%d satj=%d",sat[i],sat[j]);
			//LOG_printf(&trace,"dd HIi=%d Hij=%d",IB(sat[i],f,opt),IB(sat[j],f,opt));
                    }
                }
                else {
			...
			{
                ucErrSatCount++;
				if( (ucErrSatCount>g_tRtk.sol.ns/2)||(ucErrSatCount>5) )
				{
					g_usObsErrFlag = 1;
				}
				else
            ...
            if (opt->maxinno>0.0&&fabs(v[nv])>opt->maxinno) {
              ...
              //LOG_printf(&trace,"outlier rejected sat=%d-%d %d",sat[i],sat[j],f%nf+1);
								g_usSatNum[8]++;
                continue;
            }
        ...
        b++;
    }//end for (f=opt->mode>PMODE_DGPS?0:nf;f<nf*2;f++)
    }//end for (m=0;m<2;m++) //for each system (0:gps/qzss/sbas,1:glonass) 
    /* end of system loop */
...
//		LOG_printf(&trace,"glomodear=0");
        ...
        for (f=0,k=na;f<nf;f++,k+=MAXSAT) {
            ...
            for (i=k;i<k+MAXSAT;i++) {
                ...
                //    (m==1&&rtk->ssat[i-k].sys!=SYS_GLO)) 
                //	{
				//LOG_printf(&trace,"1 GLO SKIP");
		  //		continue;
                //	}
//----------guan 2013.09.30------
		if((rtk->x[i]==0.0)||
			((m==0)&&(rtk->ssat[i-k].sys!=SYS_GPS))||
			((m==1)&&(rtk->ssat[i-k].sys!=SYS_GLO))||
			((m==2)&&(rtk->ssat[i-k].sys!=SYS_CMP)))
			{
				continue;
			}
		
		//---------------------------------
		/*if(rtk->ssat[i-k].sys==SYS_GLO)
		{
//			LOG_printf(&trace,"m %d f%d GLO 1",m,f);
		}
		else if(rtk->ssat[i-k].sys==SYS_CMP)
		{
			LOG_printf(&trace,"m %d f%d BD 1",m,f);
		}
		*/
		//-------------------------------------
            ...
            for (j=k;j<k+MAXSAT;j++) {
                ...
                //    (m==1&&rtk->ssat[j-k].sys!=SYS_GLO)) 
                //	{
				//LOG_printf(&trace,"2 GLO SKIP");
		//		continue;
               // 	}
//----------guan 2013.09.30------
		  if(i==j||rtk->x[j]==0.0||
			((m==0)&&(rtk->ssat[j-k].sys!=SYS_GPS))||
			((m==1)&&(rtk->ssat[j-k].sys!=SYS_GLO))||
			((m==2)&&(rtk->ssat[j-k].sys!=SYS_CMP)))
			{
				continue;
			}
		  
		//---------------------------------
		/*if(rtk->ssat[j-k].sys==SYS_GLO)
		{
			LOG_printf(&trace,"m %d f%d GLO 2",m,f);
		}
		else if(rtk->ssat[j-k].sys==SYS_CMP)
		{
			LOG_printf(&trace,"m %d f%d BD 2",m,f);
		}
		*/
		//-------------------------------------
...
//    LOG_printf(&trace,"restamb nx%d na %d",rtk->nx,rtk->na);
    ...
    for (m=0;m<3;m++) {
        ...
        for (n=i=0;i<MAXSAT;i++) {
            ...
            //	{
			//LOG_printf(&trace,"GLO SKIP");
		//	continue;
            	//}

		//----------guan 2013.09.30------
		if((rtk->ssat[i].fix[f]!=2)||
		((m==0)&&(rtk->ssat[i].sys!=SYS_GPS))||
		((m==1)&&(rtk->ssat[i].sys!=SYS_GLO))||
		((m==2)&&(rtk->ssat[i].sys!=SYS_CMP)))
		{
			continue;
		}
		//-----------------------------------
            index[n++]=IB(i+1,f,&rtk->opt);
        }//end for (n=i=0;i<MAXSAT;i++)
        //LOG_printf(&trace,"restamb m %d f %d n %d",m,f,n);
        if (n<2) 
        {
//		LOG_printf(&trace,"n less 2");
		continue;
        }
...
//#pragma CODE_SECTION(holdamb,"sect_ECODE_II");
//#pragma DATA_SECTION(dMat_v_amb,"sect_EDATA_I");
//#pragma DATA_SECTION(dMat_H_amb,"sect_EDATA_I");
//#pragma DATA_SECTION(dMat_R_amb,"sect_EDATA_I");
//double dMat_v_amb[147]={0.0},dMat_H_amb[130*147]={0.0},dMat_R_amb[130*147];
static void holdamb(rtk_t *rtk, const double *xa)
{
    ...
    //trace(3,"holdamb :\n");
//    LOG_printf(&trace,"holdamb");
    v=mat(nb,1); H=zeros(nb,rtk->nx);
	//memset(dMat_H_amb,0,sizeof(dMat_H_amb));
	//v=dMat_v_amb; H=dMat_H_amb;
    
    for (m=0;m<3;m++) for (f=0;f<nf;f++) {
    ...
    if (nv>0) {
        R=zeros(nv,nv);
		//memset(dMat_R_amb,0,sizeof(dMat_R_amb));
		//R=dMat_R_amb;
...
//D要足够大(静态)，不妨设为nx*2
//int* D = new int[nx*2]; //全局变量
//#pragma CODE_SECTION(ddmat_wei,"sect_ECODE_II");
static int ddmat_wei(rtk_t *rtk, int *D)
{
	int i,j,k,m,f,nb=0,nx=rtk->nx,na=rtk->na,nf=NF(&rtk->opt);
...
//			LOG_printf(&trace,"glomodear=0");
		...
		//----------guan 2013.09.30------
		if((rtk->x[i]==0.0)||
			((m==0)&&(rtk->ssat[i-k].sys==SYS_GLO||rtk->ssat[i-k].sys==SYS_CMP))||
			((m==1)&&(rtk->ssat[i-k].sys!=SYS_GLO||rtk->ssat[i-k].sys==SYS_CMP))||
			((m==2)&&(rtk->ssat[i-k].sys!=SYS_CMP)))
			{
				continue;
			}
		...
		//---------------------------------
			...
			for (j=k;j<k+MAXSAT;j++) {
				//----------guan 2013.09.30------
			  	if(i==j||rtk->x[j]==0.0||
				((m==0)&&(rtk->ssat[j-k].sys==SYS_GLO||rtk->ssat[j-k].sys==SYS_CMP))||
				((m==1)&&(rtk->ssat[j-k].sys!=SYS_GLO||rtk->ssat[j-k].sys==SYS_CMP))||
				((m==2)&&(rtk->ssat[j-k].sys!=SYS_CMP)))
				{
					continue;
				}
...
///计算 y = D'x, Qy = D'PD
//#pragma CODE_SECTION(cal_Ds_wei,"sect_ECODE_II");
void cal_Ds_wei(int na, int nb, int nx, int ny, int* D, double* x, double* P, double* y, double* Qy)
{
	int i, j, n;
	n = na + nb;
	...
	//右上
	//M = P,  Qy(i, na+j) = M(i, D1(j)) - M(i, D2(j)) = P(i, ji1) - P(i, ji2)
	for (i=0; i<na; i++)
	{
		for (j=0; j<nb; j++)
		{
			Qy[i + (na+j)*n] = P[i + D[j*2]*nx] - P[i + D[j*2+1]*nx];
		}
	}
	//左下
	//M(na+i, j) = P(D1(i), j) - P(D2(i), j), Qy = M
	for (i=0; i<nb; i++)
	{
		for (j=0; j<na; j++)
		{
			Qy[na + i + j*n] = P[D[i*2] + j*nx] - P[D[i*2+1] + j*nx];
		}
	}
	//右下
	//M(na+m, na+k) = P(D1(m), na+k) - P(D2(m), na+k)
	//Qy(na+i, na+j) = M(na+i, D1(j)) - M(na+i, D2(j))
	//               = P(D1(i), D1(j)) - P(D2(i), D1(j)) - P(D1(i), D2(j)) + P(D2(i), D2(j));
	for (i=0; i<nb; i++)
	{
		for (j=0; j<nb; j++)
		{
...
//#pragma CODE_SECTION(resamb_LAMBDA,"sect_ECODE_II");
//#pragma DATA_SECTION(dMat_yR,"sect_EDATA_I");
//#pragma DATA_SECTION(dMat_Qy,"sect_EDATA_I");
//#pragma DATA_SECTION(dMat_bR,"sect_EDATA_I");
//#pragma DATA_SECTION(dMat_db,"sect_EDATA_I");
//#pragma DATA_SECTION(dMat_Qb,"sect_EDATA_I");
//#pragma DATA_SECTION(dMat_Qab,"sect_EDATA_I");
//#pragma DATA_SECTION(dMat_QQR,"sect_EDATA_I");
//#pragma DATA_SECTION(iMat_Dwei,"sect_EDATA_I");
//#pragma DATA_SECTION(optlambda,"sect_EDATA_I");
//double dMat_yR[1*147]={0.0},dMat_Qy[130*147]={0.0};
//double dMat_bR[2*147]={0.0},dMat_db[1*147]={0.0},dMat_Qb[130*147]={0.0},dMat_Qab[130*147]={0.0},dMat_QQR[130*147]={0.0}; 
int iMat_Dwei[130*147]={0};
prcopt_t *optlambda;
...
//   	LOG_printf(&trace,"resamb_LAMBDA nx=%d",nx);
	
	//int* D_wei = (int*)malloc(2048 * sizeof(int));	//开辟足够大的int数组，暂时1024对
	//double *y_wei, *Qy_wei;
...
//        LOG_printf(&trace,"all off");
   ...
   trace(1,"x2=\n"); tracemat(1,rtk->x,na+nb,1,2,0);
   trace(1,"p2=\n"); tracemat(1,rtk->P,nx,nx,2,0);

    matmul("TN",ny, 1,nx,1.0,D ,rtk->x,0.0,y );
    matmul("TN",ny,nx,nx,1.0,D ,rtk->P,0.0,DP);
    matmul("NN",ny,ny,nx,1.0,DP,D     ,0.0,Qy);

   trace(1,"y=\n"); tracemat(1,y,na+nb,1,2,0);
   trace(1,"Qy=\n"); tracemat(1,Qy,na+nb,na+nb,2,0);
   trace(1,"D=\n"); tracemat(1,D,nx,na+nb,2,0);   */
	/* transform single to double-differenced phase-bias (y=D'*x, Qy=D'*P*D) */
	

    //{ //added by Weizhenyi
   if ((nb=ddmat_wei(rtk,D_wei))<=0) {
        //errmsg(rtk,"no valid double-difference\n");
        return 0;
    }
	ny=na+nb; //y_wei=mat(ny,1); Qy_wei=mat(ny,ny);
	y=mat(ny,1); Qy=mat(ny,ny);
	//y=dMat_yR; Qy=dMat_Qy;

    cal_Ds_wei(na, nb, nx, ny, D_wei, rtk->x, rtk->P, y, Qy);
   //} //end
	
	b=mat(nb,2); db=mat(nb,1); Qb=mat(nb,nb); Qab=mat(na,nb); QQ=mat(na,nb);
	
	//b=dMat_bR; db=dMat_db; Qb=dMat_Qb; Qab=dMat_Qab; QQ=dMat_QQR;
...
//        LOG_printf(&trace,"pass lambda info=%d",info);
//		LOG_printf(&trace,"s1=%d s0=%d",(int)(s[1]*10),(int)(s[0]*10));
        ...
        if (s[0]<=0.0||s[1]/s[0]>=optlambda->thresar[0]) {
...
//                LOG_printf(&trace,"resamb valid ok ");
                /* restore single-differenced ambiguity */
                restamb(rtk,bias,nb,xa);
            }
            else nb=0;
        }//END if (s[0]<=0.0||s[1]/s[0]>=opt->thresar[0])
        else { /* validation failed */
            //errmsg(rtk,"ambiguity validation failed (nb=%d ratio=%.2f s=%.2f/%.2f)\n",
            //       nb,s[1]/s[0],s[0],s[1]);
//            LOG_printf(&trace,"ambiguity valid failed");
//		LOG_printf(&trace,"nb %d ratio %d",nb,(int)((s[1]/s[0])*100));
            nb=0;
        }
    }//END if (!(info=lambda(nb,2,y+na,Qb,b,s)))
    else {
        //errmsg(rtk,"lambda error (info=%d)\n",info);
//        LOG_printf(&trace,"lambda error info %d",info);
    }
	//free(D);          free(DP);
	free(y); free(Qy);
...
//#pragma CODE_SECTION(valpos,"sect_ECODE_II");
...
//    LOG_printf(&trace,"valpos nv %d thres %d",nv,(int)(thres*10));
...
//        LOG_printf(&trace,"large residual sat %d-sat %d",sat1,sat2);
    }
...
//#pragma CODE_SECTION(relpos,"sect_ECODE_II");
//#pragma DATA_SECTION(dMat_rs1,"sect_EDATA_III");
//#pragma DATA_SECTION(dMat_dts1,"sect_EDATA_III");
//#pragma DATA_SECTION(dMat_var1,"sect_EDATA_III");
//#pragma DATA_SECTION(dMat_y1,"sect_EDATA_III");
//#pragma DATA_SECTION(dMat_e1,"sect_EDATA_III");
//#pragma DATA_SECTION(dMat_zael2,"sect_EDATA_III");
//#pragma DATA_SECTION(dMat_xp1,"sect_EDATA_III");
//#pragma DATA_SECTION(dMat_Pp,"sect_EDATA_III");
//#pragma DATA_SECTION(dMat_xa,"sect_EDATA_III");
//#pragma DATA_SECTION(dMat_v2,"sect_EDATA_III");
//#pragma DATA_SECTION(dMat_H2,"sect_EDATA_III");
//#pragma DATA_SECTION(dMat_R2,"sect_EDATA_III");
//#pragma DATA_SECTION(dMat_bias,"sect_EDATA_III");
//#pragma DATA_SECTION(optrel,"sect_EDATA_III");
prcopt_t *optrel;
//double dMat_xp1[1*147]={0.0},dMat_Pp[130*147]={0.0},dMat_xa[1*147]={0.0};
//double dMat_v2[1*147]={0.0},dMat_H2[130*147]={0.0},dMat_R2[130*147]={0.0},dMat_bias[1*147]={0.0};
//double dMat_rs1[6*147]={0.0},dMat_dts1[2*147]={0.0},dMat_var1[1*147]={0.0},dMat_y1[36*147]={0.0},dMat_e1[3*147]={0.0},dMat_zael2[2*147]={0.0}; 
unsigned int uiTic[30];
                  ...
                  const nav_t *nav)
{
	...
	//uiTic[0]			= PRD_getticks();
	optrel=&rtk->opt;
//	LOG_printf(&trace,"relpos nu=%d nr=%d",nu,nr);
    dt=timediff(time,obs[nu].time);
    
    rs=mat(6,n); dts=mat(2,n); var=mat(1,n); y=mat(nf*2,n); e=mat(3,n);
	//memset(dMat_rs1,0,sizeof(dMat_rs1));
	//memset(dMat_dts1,0,sizeof(dMat_dts1));
	//memset(dMat_var1,0,sizeof(dMat_var1));
	//memset(dMat_y1,0,sizeof(dMat_y1));
	//memset(dMat_e1,0,sizeof(dMat_e1));	
	//rs=dMat_rs1; dts=dMat_dts1; var=dMat_var1; y=dMat_y1; e=dMat_e1;

    azel=zeros(2,n);
	//memset(dMat_zael2,0,sizeof(dMat_zael2));
	//azel=dMat_zael2;
    ...
    satposs(time,obs,n,nav,optrel->sateph,rs,dts,var,svh);
//	uiTic[1]			= PRD_getticks();
    /* undifferenced residuals for base station */
    if (!zdres(1,obs+nu,nr,rs+nu*6,dts+nu*2,svh+nu,nav,rtk->rb,optrel,1,
               y+nu*nf*2,e+nu*3,azel+nu*2)) {
        
//        LOG_printf(&trace,"initial base error");
...
//	uiTic[2]			= PRD_getticks();
    /* select common satellites between rover and base-station */
    if ((ns=selsat(obs,azel,nu,nr,optrel,sat,iu,ir))<=0) {
        //errmsg(rtk,"no common satellite\n");
//        LOG_printf(&trace,"no common satellite");
        free(rs); free(dts); free(var); free(y); free(e); free(azel);
        return 0;
    }
		g_usSatNum[0] = ns;
//	LOG_printf(&trace,"selsat ns %d",ns);
    /* temporal update of states */
    udstate(rtk,obs,sat,iu,ir,ns,nav);
		g_usSatNum[1] = ns;
    
    //trace(4,"x(0)="); tracemat(4,rtk->x,1,NR(optrel),13,4);
    
    xp=mat(rtk->nx,1); Pp=zeros(rtk->nx,rtk->nx); xa=mat(rtk->nx,1);
	//memset(dMat_xp1,0,sizeof(dMat_xp1));
	//memset(dMat_Pp,0,sizeof(dMat_Pp));	
	//memset(dMat_xa,0,sizeof(dMat_xa));	
	//xp=dMat_xp1; Pp=dMat_Pp; xa=dMat_xa;
    matcpy(xp,rtk->x,rtk->nx,1);
    
    ny=ns*nf*2+2;
    v=mat(ny,1); H=zeros(rtk->nx,ny); R=mat(ny,ny); bias=mat(rtk->nx,1);
	//memset(dMat_v2,0,sizeof(dMat_v2));
	//memset(dMat_H2,0,sizeof(dMat_H2));
	//memset(dMat_R2,0,sizeof(dMat_R2));
	//memset(dMat_bias,0,sizeof(dMat_bias));
	
	//v=dMat_v2; H=dMat_H2; R=dMat_R2; bias=dMat_bias;
    
    /* add 2 iterations for baseline-constraint moving-base */
    niter=optrel->niter+(optrel->mode==PMODE_MOVEB&&optrel->baseline[0]>0.0?2:0);
//    LOG_printf(&trace,"niter %d",niter);
	
    for (i=0;i<niter;i++) {
        /* undifferenced residuals for rover */
        if (!zdres(0,obs,nu,rs,dts,svh,nav,xp,optrel,0,y,e,azel)) {
            //errmsg(rtk,"rover initial position error\n");
//            LOG_printf(&trace,"rover initial position error");
            stat=SOLQ_NONE;
            break;
        }

//		LOG_printf(&trace,"partial deriva");
        /* double-differenced residuals and partial derivatives */
        if ((nv=ddres(rtk,nav,dt,xp,Pp,sat,y,e,azel,iu,ir,ns,v,H,R,vflg))<1) {
            //errmsg(rtk,"no double-differenced residual\n");
//            LOG_printf(&trace,"no dd residual nv %d",nv);
            stat=SOLQ_NONE;
            break;
        }
//		LOG_printf(&trace,"dd nv %d",nv);
        /* kalman filter measurement update */
        matcpy(Pp,rtk->P,rtk->nx,rtk->nx);
//	uiTic[3]			= PRD_getticks();
        //filter_start=LED_flag;
        if ((info=filter(xp,Pp,H,v,R,rtk->nx,nv))) {
		//if ((info=filter_wei(xp,Pp,H,v,R,rtk->nx,nv))) {
		
            //errmsg(rtk,"filter error (info=%d)\n",info);
            //LOG_printf(&trace,"filter error info%d",info);
            stat=SOLQ_NONE;
            break;
        }
//        filter_end=LED_flag;
	
        //trace(4,"x(%d)=",i+1); tracemat(4,xp,1,NR(optrel),13,4);
    }//END for (i=0;i<niter;i++)
	
//	uiTic[4]			= PRD_getticks();
    if (stat!=SOLQ_NONE&&zdres(0,obs,nu,rs,dts,svh,nav,xp,optrel,0,y,e,azel)) {

//		LOG_printf(&trace,"valid of float");
        ...
        if (valpos(rtk,v,R,vflg,nv,4.0)) {
			...
			//LOG_printf(&trace,"float sat %d",sat[i]);
                }
    ...
    }//end  if (stat!=SOLQ_NONE&&zdres(0,obs,nu,rs,dts,svh,nav,xp,optrel,0,y,e,azel))
	g_usSatNum[2] = rtk->sol.ns;
    /* resolve integer ambiguity by WL-NL 
    if (stat!=SOLQ_NONE&&rtk->opt.modear==4){//ARMODE_WLNL) {
	...
	else if (stat!=SOLQ_NONE&&rtk->opt.modear==5){//ARMODE_TCAR) {
...
//	uiTic[5]			= PRD_getticks();
    //--------------GUAN---------------
    	if(stat!=SOLQ_NONE)
    	{
    		lambda_stat=resamb_LAMBDA(rtk,bias,xa);
//		LOG_printf(&trace,"lambda_stat %d",lambda_stat);
    	}
	//--------------------------------
	if (stat!=SOLQ_NONE&&(lambda_stat>1)) {
    //if (stat!=SOLQ_NONE&&resamb_LAMBDA(rtk,bias,xa)>1) {
//        LOG_printf(&trace,"pass LAMBDA");
        if (zdres(0,obs,nu,rs,dts,svh,nav,xa,optrel,0,y,e,azel)) {

//			LOG_printf(&trace,"valid of fixed");
            ...
            if (valpos(rtk,v,R,vflg,nv,4.0)) {
                ...
                stat=SOLQ_FIX;
//			LOG_printf(&trace,"SOLQ_FIX");
            }
        }
    }//end if (stat!=SOLQ_NONE&&resamb_LAMBDA(rtk,bias,xa)>1)
	
//	uiTic[6]			= PRD_getticks();
...
* return : none
*-----------------------------------------------------------------------------*/
//#pragma CODE_SECTION(rtkinit,"sect_ECODE_II");
//#pragma DATA_SECTION(dMat_x_rtk,"sect_EDATA_I");
//#pragma DATA_SECTION(dMat_P_rtk,"sect_EDATA_I");
//#pragma DATA_SECTION(dMat_xa_rtk,"sect_EDATA_I");
//#pragma DATA_SECTION(dMat_Pa_rtk,"sect_EDATA_I");
//double dMat_x_rtk[147],dMat_P_rtk[130*147],dMat_xa_rtk[20],dMat_Pa_rtk[400];
extern void rtkinit(rtk_t *rtk, const prcopt_t *opt)
{
    ...
    //trace(3,"rtkinit :\n");
	//memset(dMat_x_rtk,0,sizeof(dMat_x_rtk));
//	memset(dMat_P_rtk,0,sizeof(dMat_P_rtk));
	//memset(dMat_xa_rtk,0,sizeof(dMat_xa_rtk));
	//memset(dMat_Pa_rtk,0,sizeof(dMat_Pa_rtk));
    ...
    rtk->opt=*opt;
		rtk->opt.posopt[4] = 1;// 需要开的,可能坏星(数据错误很大,但是rtk检测不出来)
		/*rtk->opt.snrmask.mask[0][0] = 32.0;
		rtk->opt.snrmask.mask[0][1] = 32.0;
		rtk->opt.snrmask.mask[0][2] = 32.0;
		rtk->opt.snrmask.mask[0][3] = 32.0;
		rtk->opt.snrmask.mask[0][4] = 32.0;
		rtk->opt.snrmask.mask[0][5] = 32.0;
		rtk->opt.snrmask.mask[0][6] = 32.0;
		rtk->opt.snrmask.mask[0][7] = 32.0;
		rtk->opt.snrmask.mask[0][8] = 32.0;*/
...
* return : none
*-----------------------------------------------------------------------------*/
//#pragma CODE_SECTION(rtkfree,"sect_ECODE_II");
...
*-----------------------------------------------------------------------------*/
//#pragma CODE_SECTION(rtkpos,"sect_ECODE_II");
...
//    LOG_printf(&trace,"rtkpos  rover=%d base=%d\n",nu,nr);
    time=rtk->sol.time; /* previous epoch */
		
    //dElevMin 				=  rtk->opt.elmin;	// 保存设置的仰角门限
	//	rtk->opt.elmin 	= 9.0*D2R;					// 单点定位使用9度作为仰角门限,为了在受干扰的时候,能出个单点
		// ublox用的是卡尔曼,这个最小二乘,所以这个单点飘逸会大一些
    ...
    if (!pntpos(obs,nu,nav,&rtk->opt,&rtk->sol,NULL,rtk->ssat,msg)) {
        ...
        if (!rtk->opt.dynamics) {
            //outsolstat(rtk);
//            LOG_printf(&trace,"dynamics 0");
            return 0;
        }
    }
		
		//rtk->opt.elmin 	= dElevMin;   // RTK使用设置的仰角门限
    if (time.time!=0) rtk->tt=timediff(rtk->sol.time,time);
    
    /* single point positioning */
    if (opt->mode==PMODE_SINGLE) {
        //outsolstat(rtk);
		//        LOG_printf(&trace,"single pos finish");
        return 1;
    }
	//	LOG_printf(&trace,"null single pos finish");
    ...
    if (nr==0) {
        ...
        //outsolstat(rtk);
        //LOG_printf(&trace,"no base station observation data for rtk");
        return 1;
    }
    ...
    if (opt->mode==PMODE_MOVEB) { /*  moving baseline */
        ...
        if (!pntpos(obs+nu,nr,nav,&rtk->opt,&solb,NULL,NULL,msg)) {
			...
			//            LOG_printf(&trace,"base station position error");
            return 0;
        }
        ...
        if (fabs(rtk->sol.age)>opt->maxtdiff) {
            ...
            //outsolstat(rtk);
						memset(g_usCommonSat,0,sizeof(g_usCommonSat));
			//            LOG_printf(&trace,"age of diff error=%d",(int)(rtk->sol.age*10));
            return 1;
        }
    }
	
	//	LOG_printf(&trace,"NF=%d",NF(opt));
	//	LOG_printf(&trace,"NP=%d",NP(opt));
	//	LOG_printf(&trace,"NI=%d",NI(opt));
	//	LOG_printf(&trace,"NT=%d",NT(opt));
	//	LOG_printf(&trace,"NL=%d",NL(opt));
	//	LOG_printf(&trace,"NB=%d",NB(opt));
	//	LOG_printf(&trace,"NR=%d",NR(opt));
	//	LOG_printf(&trace,"NX=%d",NX(opt));
	//if(rtk->opt.navsys==SYS_GPS|SYS_CMP|SYS_GLO)
	//return 1;
